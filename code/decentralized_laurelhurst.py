...
LAURELHURST_CONFIG = DecentralizedConfig(
    name="Laurelhurst",
    survey_path="data/laurelhurst_survey_with_address.csv",
    residents_path="data/laurelhurst_address.csv",
    distance_matrix_path="data/laurelhurst_distance_matrix.npy",
...
# ---------------------------------------------------------------------------
# Environment building and utility functions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Sharing LP and queue simulation
# ---------------------------------------------------------------------------
...
def save_decentralized_figure(results_df: pd.DataFrame, env: DecentralizedEnvironment, captain_folder: str, filename: str, root_dir: str = "figures") -> None:
    output_dir = Path(root_dir) / "decentralized" / env.config.name / captain_folder
    output_dir.mkdir(parents=True, exist_ok=True)

    fig, ax = plt.subplots(figsize=(8, 6))
    ax.plot(results_df["n_captains"], results_df["log10_avg_deprivation_cost"], "-o", label=f"Decentralized ({env.config.name})")
    ax.set_xlabel("Number of sharing captains")
    ax.set_ylabel("log10 Deprivation cost")
    ax.legend()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Laurelhurst decentralized sharing model.")
